...
date = '02232022';
fnames = [2 4 5]; %MAKE MORE FLEXIBLE-> NEED TO BATCH PROCESS EVENTUALLY
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%find images and 
cd(['D:\BRUKER\',date])
...
%         end
    ...
    for j = 1:length(batches)
        ...
        for frmn = 1:size(imgStack,3)
            ...
            filenameCh1 = [outputDirCh1 '\' sprintf('%06i',ImgCnt) '.tif'];
            filenameCh2 = [outputDirCh2 '\' sprintf('%06i',ImgCnt) '.tif'];
            
            if useCh2template
               
                imwrite(uint16(ch1Stack(:,:,frmn))',filenameCh1,'tif','writemode','overwrite');
                imwrite(uint16(imgStack(:,:,frmn))',filenameCh2,'tif','writemode','overwrite');
               
            else

                imwrite(uint16(imgStack(:,:,frmn))',filenameCh1,'tif','writemode','overwrite');

            end
        end
        disp(['Saved Images: ',filenameCh1]);
        toc;
    end
end
clear 
